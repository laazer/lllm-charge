...
export class CommonCommandHandler {
    ...
    async handleCommand(input, cwd = process.cwd()) {
    ...
    async handleGitCommitAndPush(match, cwd) {
    ...
    async handleGitAddAll(match, cwd) {
    ...
    async handleGitStatus(match, cwd) {
    ...
    async handleCreateBranch(match, cwd) {
    ...
    async handleNpmInstall(match, cwd) {
    ...
    async handleNpmBuild(match, cwd) {
    ...
    async handleListFiles(match, cwd) {
    ...
    async handleCreateFileOrDir(match, cwd) {
        const name = match[1];
    ...
    async handleDeleteFile(match, cwd) {
        const name = match[1];
        const fullPath = path.resolve(cwd, name);
        try {
    ...
    async handleKillPort(match, cwd) {
        const port = match[1];
        try {
    ...
    async handleCheckPort(match, cwd) {
        const port = match[1];
        try {
    ...
    async handleDiskUsage(match, cwd) {
        const directory = match[1] || '.';
    ...
    async handleFindFiles(match, cwd) {
    ...
    async handleCurrentDirectory(match, cwd) {
        return {
            success: true,
            output: cwd,
            command: 'pwd',
    ...
    async handleDockerCommand(match, cwd) {
    ...
    async handleNodeVersion(match, cwd) {
    ...
    async handleNpmVersion(match, cwd) {
    ...
    // CLI tool handlers
    async handleGitHubCLI(match, cwd) {
    ...
    async handleFileView(match, cwd) {
        const fullMatch = match[0];
        const result = execSync(fullMatch, { cwd, encoding: 'utf-8' });
        return {
            success: true,
    ...
    async handleProcessCommands(match, cwd) {
        const fullMatch = match[0];
        const result = execSync(fullMatch, { cwd, encoding: 'utf-8' });
        return {
            success: true,
    ...
    async handleFileCopy(match, cwd) {
        const fullMatch = match[0];
        const result = execSync(fullMatch, { cwd, encoding: 'utf-8' });
        return {
            success: true,
    ...
    async handleFilePermissions(match, cwd) {
        const fullMatch = match[0];
        const result = execSync(fullMatch, { cwd, encoding: 'utf-8' });
        return {
            success: true,
    ...
    async handleDownload(match, cwd) {
        const fullMatch = match[0];
        const result = execSync(fullMatch, { cwd, encoding: 'utf-8' });
        return {
            success: true,
    ...
    async handleShellInfo(match, cwd) {
        const fullMatch = match[0];
        const result = execSync(fullMatch, { cwd, encoding: 'utf-8' });
        return {
            success: true,
