...
program
    ...
    try {
        ...
        if (options.workload !== 'general') {
            ...
            const workloadOpt = await optimizer.optimizeForWorkload(options.workload);
...
program
    ...
    try {
        ...
        await fs.writeFile(path.join(projectPath, '.claude.json'), JSON.stringify(mcpConfig, null, 2));
...
async function loadConfig(configPath, projectPath) {
