...
export class DocsIntelligence {
    ...
    smartCache;
    cachedDocs = new Map();
    constructor(projectDir, knowledgeBase) {
    ...
    async getAvailableDocumentations() {
    ...
    async searchSemanticDocs(query) {
        try {
    ...
    async searchDevDocs(query) {
        try {
            ...
            for (const doc of availableDocs) {
                ...
                const nameMatch = searchTerms.some(term => doc.name.toLowerCase().includes(term) ||
    ...
    async downloadDocumentation(docName) {
        try {
    ...
    calculateKeywordSimilarity(query, target) {
