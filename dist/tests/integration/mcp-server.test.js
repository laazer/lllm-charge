...
export default App
    ...
    describe('Intelligence Tools', () => {
        beforeEach(async () => {
            ...
            server.intelligence = mockIntelligence;
        });
        it('should build context packages', async () => {
            const handler = server.handleBuildContextPackage.bind(server);
        ...
        it('should search code symbols', async () => {
            const handler = server.handleSearchCodeSymbols.bind(server);
    ...
    describe('Documentation Tools', () => {
        beforeEach(async () => {
            ...
            server.docsIntelligence = mockDocsIntelligence;
        });
        it('should search developer documentation', async () => {
            const handler = server.handleDocsTool.bind(server);
    ...
    describe('Reasoning Tools', () => {
        beforeEach(async () => {
            ...
            server.reasoning = mockReasoning;
            server.rlmEngine = mockRLMEngine;
        });
        it('should handle hybrid reasoning requests', async () => {
            const handler = server.handleHybridReasoning.bind(server);
        ...
        it('should start RLM reasoning sessions', async () => {
            const handler = server.handleStartRLMSession.bind(server);
            const result = await handler({
    ...
    describe('Common Command Tools', () => {
        beforeEach(async () => {
            ...
            server.commandHandler = mockCommandHandler;
        });
        it('should execute common commands', async () => {
            const handler = server.handleExecuteCommonCommand.bind(server);
            const result = await handler({
                command: 'git status',
        ...
        it('should list available commands', async () => {
            const handler = server.handleListAvailableCommands.bind(server);
            const result = await handler({});
            expect(result).toHaveProperty('content');
    ...
    describe('Cost Tracking', () => {
        beforeEach(async () => {
            ...
            server.costTracker = mockCostTracker;
        });
        it('should provide cost metrics', async () => {
            const handler = server.handleGetCostMetrics.bind(server);
        ...
        it('should provide optimization recommendations', async () => {
            const handler = server.handleOptimizeLocalUsage.bind(server);
    ...
    describe('System Status', () => {
        it('should provide comprehensive system status', async () => {
            ...
            jest.spyOn(server, 'getSystemStatus').mockResolvedValue(mockStatus);
            const handler = server.handleGetSystemStatus.bind(server);
            const result = await handler({});
            expect(result).toHaveProperty('content');
    ...
    describe('Error Handling', () => {
        ...
        it('should handle unknown tools', async () => {
            ...
            try {
                await server.server.requestHandler(mockRequest);
            }
            catch (error) {
    ...
    describe('Performance and Scalability', () => {
        it('should handle concurrent requests efficiently', async () => {
            ...
            const promises = Array.from({ length: 10 }, () => handler({ query: 'concurrent test', maxTokens: 1000 }));
