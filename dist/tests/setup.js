...
if (typeof expect !== 'undefined') {
    expect.extend({
        ...
        toHaveValidCostMetrics(received) {
...
export class MockLLMProvider {
    costPerToken = 0.0015 / 1000; // Approximate cost per token
...
export async function setupTestDirectories() {
    try {
        ...
        await fs.writeFile(path.join(TEST_CONFIG.TEST_PROJECT_DIR, 'package.json'), JSON.stringify({
            name: 'test-project',
            version: '1.0.0',
            dependencies: {
                'react': '^18.0.0',
        ...
        await fs.writeFile(path.join(TEST_CONFIG.TEST_PROJECT_DIR, 'src', 'index.ts'), 'console.log("Hello, World!")');
    }
    catch (error) {
...
export function generateTestQuery(type = 'simple') {
    switch (type) {
        case 'simple':
...
// Validation helpers
export function validateCostMetrics(metrics) {
