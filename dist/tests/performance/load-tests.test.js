...
describe('LLM-Charge Performance Tests', () => {
    ...
    describe('Command Handler Performance', () => {
        ...
        it('should handle command pattern matching efficiently', async () => {
            ...
            const startTime = Date.now();
            const results = await Promise.all(randomCommands.map(cmd => handler.handleCommand(cmd, testProjectDir)));
    ...
    describe('Knowledge Base Performance', () => {
        ...
        beforeEach(async () => {
            ...
            jest.spyOn(knowledgeBase, 'getOrCreateEmbedding')
                .mockImplementation((text) => mockProvider.generateEmbedding(text));
        ...
        it('should handle large-scale document storage efficiently', async () => {
            ...
            for (let i = 0; i < documentCount; i++) {
                ...
                storePromises.push(knowledgeBase.store(`doc-${i}`, content, {
                    type: 'test',
                    index: i,
        ...
        it('should handle concurrent read/write operations', async () => {
            ...
            for (let i = 0; i < concurrentOperations; i++) {
                if (i % (readsPerWrite + 1) === 0) {
                    // Write operation
                    operations.push(knowledgeBase.store(`concurrent-doc-${i}`, `Content for document ${i}`, { type: 'concurrent' }));
                }
                else {
                    // Read operation
                    operations.push(knowledgeBase.searchSemantic(`document ${Math.floor(i / 4)}`, { limit: 5 }));
                }
            }
            const startTime = Date.now();
        ...
        it('should maintain performance with large embeddings cache', async () => {
            ...
            for (let i = 0; i < cacheSize; i++) {
                await knowledgeBase.getOrCreateEmbedding(`test embedding ${i}`);
            }
            const cachingTime = Date.now() - startTime;
            // Test cache performance
            const cacheTestStart = Date.now();
            const cachedResults = await Promise.all([
                knowledgeBase.getOrCreateEmbedding('test embedding 100'),
                knowledgeBase.getOrCreateEmbedding('test embedding 500'),
                knowledgeBase.getOrCreateEmbedding('test embedding 1000')
            ]);
    ...
    describe('Smart Documentation Cache Performance', () => {
        it('should handle rapid query processing', async () => {
            ...
            const smartCache = new SmartDocsCache(mockDocsIntelligence, mockKnowledgeBase, testProjectDir);
