...
describe('LocalLLMRouter', () => {
    ...
    describe('Provider Management', () => {
        ...
        it('should handle provider failures gracefully', async () => {
            ...
            jest.spyOn(router, 'callOllamaAPI')
    ...
    describe('Performance Optimization', () => {
        ...
        it('should implement request queuing for resource management', async () => {
            ...
            const requests = Array.from({ length: 20 }, (_, i) => router.generateResponse({
    ...
    describe('Load Balancing', () => {
        it('should distribute requests across available providers', async () => {
            ...
            const responses = await Promise.all(requests.map(req => router.generateResponse(req)));
        ...
        it('should respect provider capacity limits', async () => {
            ...
            const requests = Array.from({ length: 8 }, (_, i) => router.generateResponse({
                prompt: `Test prompt ${i}`,
                task: 'simple_qa'
            }));
            const startTime = Date.now();
        ...
        it('should adaptively adjust routing based on performance', async () => {
            ...
            const responses = await Promise.all(Array.from({ length: 5 }, () => router.generateResponse({
                prompt: 'Test prompt',
                preferredProvider: 'auto'
            })));
    ...
    describe('Error Handling and Recovery', () => {
        it('should handle provider timeouts gracefully', async () => {
            // Mock a timeout
            jest.spyOn(router, 'callOllamaAPI')
                .mockImplementation(() => new Promise((_, reject) => setTimeout(() => reject(new Error('Request timeout')), 100)));
            const response = await router.generateResponse({
