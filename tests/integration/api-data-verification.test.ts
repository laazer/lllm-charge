 ...
 * and that components can process this data correctly
 */

const fetch = require('node-fetch')

// Add fetch as a global for the test environment
global.fetch = fetch
  ...
  describe('Real API Data Tests', () => {
    test('should return real agents data (independent of projects)', async () => {
      ...
      agents.forEach(agent => {
        ...
        // Agents should be independent - no projectId field or it should be null
        expect(agent.projectId).toBeUndefinedOrNull()
      })
      
      // Verify specific agent data structure matches independent architecture
...
// Custom matcher for projectId that can be undefined or null
expect.extend({
  toBeUndefinedOrNull(received) {
