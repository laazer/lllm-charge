...
import { UnifiedIntelligence } from '../../src/intelligence/unified-intelligence'
...
import { TEST_CONFIG, createMockConfig, MockLLMProvider, delay } from '../setup'
...
describe('End-to-End LLM-Charge Integration', () => {
    ...
    it('should handle a typical development query workflow', async () => {
      ...
      for (const step of workflow) {
        ...
        if (step.tool === 'execute_common_command') {
          result = await (server as any).handleExecuteCommonCommand(step.args)
        } else if (step.tool === 'search_developer_docs') {
          result = await (server as any).handleDocsTool('search_developer_docs', step.args)
        } else if (step.tool === 'build_context_package') {
          result = await (server as any).handleBuildContextPackage(step.args)
        } else if (step.tool === 'get_cost_metrics') {
          result = await (server as any).handleGetCostMetrics(step.args)
        }
  ...
  describe('Smart Documentation Integration', () => {
    it('should auto-detect and cache documentation needs', async () => {
      ...
      jest.spyOn(docsIntelligence as any, 'setupDevDocs').mockResolvedValue()
      jest.spyOn(docsIntelligence as any, 'setupGPT4All').mockResolvedValue()
      jest.spyOn(docsIntelligence as any, 'loadAvailableDocs').mockResolvedValue([])
      ...
      for (const { query, expectedDocs } of testQueries) {
        ...
        const mockSearchResults = expectedDocs.map(doc => ({
          ...
          type: 'doc',
          similarity: 0.9,
          source: 'devdocs' as const
        }))

        jest.spyOn(docsIntelligence as any, 'searchSemanticDocs').mockResolvedValue(mockSearchResults)
        jest.spyOn(docsIntelligence as any, 'searchDevDocs').mockResolvedValue([])
  ...
  describe('Performance and Scalability', () => {
    it('should handle high-volume operations efficiently', async () => {
      ...
      const promises = operations.map(op => 
        handler.handleCommand(op.command, testProjectDir)
      )
  ...
  describe('Error Recovery and Resilience', () => {
    it('should recover gracefully from component failures', async () => {
      ...
      const failedResult = await (server as any).handleBuildContextPackage({ query: 'test' })
      expect(failedResult.isError).toBe(true)

      // Second call should succeed
      const successResult = await (server as any).handleBuildContextPackage({ query: 'test' })
  ...
  // Helper functions
  async function createTestProject(projectDir: string) {
    // Create package.json
    await fs.writeFile(
...
app.listen(3000, () => {
  ...
  async function mockServerComponents(server: LLMChargeServer) {
    ...
    ;(server as any).commandHandler = new CommonCommandHandler()
    ;(server as any).costTracker = {
