...
import { N8nWorkflowEngine, WorkflowDefinition, WorkflowNode, ExecutionStatus } from '../../../src/workflows/n8n-workflow-engine'

describe('N8nWorkflowEngine', () => {
  let engine: N8nWorkflowEngine
  let mockWorkflow: Omit<WorkflowDefinition, 'id'>
  ...
  describe('Node Type Management', () => {
    test('should register new node type', () => {
      const mockNodeType = {
        ...
        inputs: [{ displayName: 'Input', type: 'main' as const }],
        outputs: [{ displayName: 'Output', type: 'main' as const }],
        properties: []
      }
  ...
  describe('LLM-Specific Methods', () => {
    ...
    test('should cleanup temporary workflow after LLM task', async () => {
      const initialWorkflows = (await Promise.all(
        Array.from({ length: 100 }, (_, i) => engine.getWorkflow(`workflow_${i}`))
      )).filter(Boolean)
      ...
      const finalWorkflows = (await Promise.all(
        Array.from({ length: 100 }, (_, i) => engine.getWorkflow(`workflow_${i}`))
      )).filter(Boolean)
  ...
  describe('Performance and Memory Management', () => {
    test('should handle multiple concurrent executions', async () => {
      ...
      const executions = await Promise.all(
        executionIds.map(id => engine.getExecution(id))
      )
