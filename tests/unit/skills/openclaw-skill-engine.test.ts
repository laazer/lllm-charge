...
describe('OpenClawSkillEngine', () => {
  let skillEngine: OpenClawSkillEngine
  ...
  describe('Skill Installation', () => {
    test('should install skill from SKILL.md file', async () => {
      const skillRequest: SkillInstallRequest = {
        name: 'code-analyzer',
        source: 'local',
...
`
      ...
      ;(fs.readFile as jest.Mock).mockResolvedValue(mockSkillContent)
    ...
    test('should emit skill-installed event', async () => {
      ...
      const fs = await import('fs/promises')
      ;(fs.readFile as jest.Mock).mockResolvedValue('# Event Test Skill\n## Commands\n- test: Test command')
      ;(fs.stat as jest.Mock).mockResolvedValue({ isFile: () => true })
...
`
      ...
      ;(fs.readFile as jest.Mock).mockResolvedValue(invalidSkillContent)
      ;(fs.stat as jest.Mock).mockResolvedValue({ isFile: () => true })

      await expect(
        skillEngine.installSkill('invalid-skill', '/test/invalid/SKILL.md')
      ).rejects.toThrow('Invalid skill definition: No commands found')
    })
  })
  ...
  describe('Skill Execution', () => {
    ...
    beforeEach(async () => {
      ...
      const fs = await import('fs/promises')
      ;(fs.readFile as jest.Mock).mockResolvedValue(JSON.stringify(testSkill))
      ;(fs.stat as jest.Mock).mockResolvedValue({ isFile: () => true })
    ...
    test('should enforce execution timeouts', async () => {
      ...
      const longRunningSkill: SkillDefinition = {
        ...testSkill,
      ...
      const fs = await import('fs/promises')
      ;(fs.readFile as jest.Mock).mockResolvedValue(JSON.stringify(longRunningSkill))
    ...
    test('should enforce memory limits', async () => {
      const memoryLimitSkill: SkillDefinition = {
        ...testSkill,
      ...
      const fs = await import('fs/promises')
      ;(fs.readFile as jest.Mock).mockResolvedValue(JSON.stringify(memoryLimitSkill))
  ...
  describe('Sandbox Security', () => {
    test('should prevent file access outside allowed paths', async () => {
      const restrictedSkill: SkillDefinition = {
      ...
      const fs = await import('fs/promises')
      ;(fs.readFile as jest.Mock).mockResolvedValue(JSON.stringify(restrictedSkill))
    ...
    test('should prevent network access when disabled', async () => {
      const noNetworkSkill: SkillDefinition = {
      ...
      const fs = await import('fs/promises')
      ;(fs.readFile as jest.Mock).mockResolvedValue(JSON.stringify(noNetworkSkill))
  ...
  describe('Skill Management', () => {
    ...
    test('should update skill to new version', async () => {
      ...
      const fs = await import('fs/promises')
      ;(fs.readFile as jest.Mock).mockResolvedValue(JSON.stringify(skillV1))
      ...
      ;(fs.readFile as jest.Mock).mockResolvedValue(JSON.stringify(skillV2))
  ...
  describe('Skill Discovery', () => {
    test('should discover skills in directory', async () => {
      const fs = await import('fs/promises')
      ;(fs.readdir as jest.Mock).mockResolvedValue(['skill1', 'skill2', 'skill3'])
