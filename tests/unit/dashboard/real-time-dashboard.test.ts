...
describe('RealTimeDashboard', () => {
  ...
  describe('Dashboard Initialization', () => {
    ...
    test('should load saved widgets on startup', async () => {
      ...
      ;(fs.readFile as jest.Mock).mockResolvedValue(JSON.stringify(mockWidgets))
  ...
  describe('Real-Time Metrics', () => {
    ...
    test('should track cost metrics over time', async () => {
      ...
      expect(costMetrics.breakdown[0]).toEqual(
        expect.objectContaining({
    ...
    test('should broadcast updates to connected clients', async () => {
      ...
      dashboard.subscribeToUpdates(ws as any)

      await dashboard.recordCostEvent('test-call', 0.01, 'test-model')

      expect(ws.send).toHaveBeenCalled()
      const sentData = JSON.parse((ws.send as jest.Mock).mock.calls[0][0])
    ...
    test('should import dashboard configuration', async () => {
      const importData = {
        ...
        alerts: [{
          ...
          condition: { metric: 'errors', operator: 'greater-than' as const, threshold: 5 },
  ...
  describe('WebSocket Communication', () => {
    ...
    test('should handle client subscriptions', async () => {
      ...
      dashboard.handleClientMessage(ws as any, JSON.stringify({
        type: 'subscribe',
    ...
    test('should handle WebSocket disconnections', async () => {
      ...
      dashboard.handleClientDisconnect(ws as any)
