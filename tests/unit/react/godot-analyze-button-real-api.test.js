...
function createFetchMock(overrides = {}) {
    return jest.fn((url) => {
        ...
        if (url.includes('/mcp/tools'))
            return Promise.resolve({ ok: true, json: () => Promise.resolve(makeToolsList()) });
        if (url.includes('/mcp/status'))
            return Promise.resolve({ ok: true, json: () => Promise.resolve(makeMCPStatus()) });
        if (url.includes('/mcp/call/godot_project_analyzer'))
            return Promise.resolve({ ok: true, json: () => Promise.resolve(makeProjectResult()) });
