...
jest.mock('../../../src/react/components/layout/Navigation/NavigationButton', () => ({
    NavigationButton: ({ id, label, isActive, onClick }) => (<button data-testid={`nav-button-${id}`} onClick={onClick} className={isActive ? 'active' : ''}>
      {label}
    </button>)
}));
describe('Overflow Menu Integration Tests', () => {
    ...
    test('NavigationBar renders overflow menu with correct positioning', () => {
        render(<NavigationBar currentSection="overview" onSectionChange={jest.fn()}/>);
    ...
    test('NavigationBar overflow menu shows secondary items when clicked', async () => {
        ...
        render(<NavigationBar currentSection="overview" onSectionChange={mockSectionChange}/>);
    ...
    test('DashboardHeader overflow menu has different positioning', () => {
        ...
        const headerOverflowButton = headerMoreButtons.find(button => button.querySelector('svg') && !button.textContent?.includes('More'));
    ...
    test('OverflowMenu component handles positioning props correctly', () => {
        const mockItems = [
            ...
            { id: 'test2', label: 'Test 2', onClick: jest.fn(), variant: 'success' }
        ];
        const { rerender } = render(<OverflowMenu items={mockItems} buttonLabel="Test More" position="bottom-right"/>);
        ...
        rerender(<OverflowMenu items={mockItems} buttonLabel="Test More" position="bottom-left"/>);
        fireEvent.click(button);
    ...
    test('OverflowMenu handles keyboard navigation and accessibility', () => {
        ...
        render(<OverflowMenu items={mockItems} buttonLabel="Test More"/>);
        const button = screen.getByText('Test More');
