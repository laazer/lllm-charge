...
describe('WebSocket Store', () => {
    ...
    describe('WebSocketProvider', () => {
        it('should establish WebSocket connection on mount', async () => {
            const wrapper = ({ children }) => (<WebSocketProvider wsUrl="ws://localhost:3001">
          {children}
        </WebSocketProvider>);
        ...
        it('should handle connection errors gracefully', async () => {
            const wrapper = ({ children }) => (<WebSocketProvider wsUrl="ws://invalid-url:3001">
          {children}
        </WebSocketProvider>);
            const { result } = renderHook(() => useWebSocket(), { wrapper });
        ...
        it('should attempt reconnection with exponential backoff', async () => {
            jest.useFakeTimers();
            const wrapper = ({ children }) => (<WebSocketProvider wsUrl="ws://localhost:3001" reconnectInterval={1000} maxReconnectAttempts={3}>
          {children}
        </WebSocketProvider>);
            const { result } = renderHook(() => useWebSocket(), { wrapper });
    ...
    describe('Error handling', () => {
        ...
        it('should handle unknown message types', async () => {
          ...
          {children}
            ...
            await waitFor(() => {
                expect(consoleLogSpy).toHaveBeenCalledWith('Unknown WebSocket message type:', 'unknown_type');
            });
        ...
        it('should handle error messages from server', async () => {
          ...
          {children}
            ...
            await waitFor(() => {
                expect(consoleErrorSpy).toHaveBeenCalledWith('WebSocket error message:', 'Server error occurred');
            });
            consoleErrorSpy.mockRestore();
        });
    });
