...
function createFetchMock(overrides: Record<string, any> = {}) {
  return jest.fn((url: string) => {
    ...
    if (url.includes('/mcp/tools')) return Promise.resolve({ ok: true, json: () => Promise.resolve(mockToolsList) })
    if (url.includes('/mcp/status')) return Promise.resolve({ ok: true, json: () => Promise.resolve(mockMCPStatus) })
    if (url.includes('/mcp/call/godot_project_analyzer')) return Promise.resolve({ ok: true, json: () => Promise.resolve(mockProjectData) })
    ...
    test('Run Tool button calls the API with projectPath', async () => {
      ...
      await waitFor(() => {
        const calls = freshFetch.mock.calls.filter(
          (call: any[]) => typeof call[0] === 'string' && call[0].includes('/mcp/call/godot_scene_analyzer')
        )
