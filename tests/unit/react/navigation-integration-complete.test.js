...
jest.mock('../../../src/react/components/ui/Menus/OverflowMenu', () => ({
    OverflowMenu: ({ items, buttonLabel, onItemClick }) => (<div data-testid="overflow-menu">
      <button data-testid="overflow-button">{buttonLabel}</button>
      <div data-testid="overflow-items">
        {items.map((item) => (<button key={item.id} data-testid={`overflow-item-${item.id}`} onClick={() => item.onClick?.()} className={item.variant === 'success' ? 'active-item' : ''}>
            {item.label}
          </button>))}
      </div>
    </div>)
...
describe('Navigation Integration Complete Tests', () => {
    ...
    const renderNavigation = (currentSection = 'overview') => {
        ...
        <NavigationBar currentSection={currentSection} onSectionChange={mockOnSectionChange}/>
      </MemoryRouter>);
    };
    ...
    describe('Navigation Interaction', () => {
        ...
        test('should dispatch custom sectionChange event', () => {
            ...
            expect(window.dispatchEvent).toHaveBeenCalledWith(new CustomEvent('sectionChange', { detail: { section: 'agents' } }));
        });
    });
    ...
    describe('Navigation Item Descriptions', () => {
        test('should have correct description for each primary item', () => {
            ...
            const primaryButtons = buttons.filter(btn => ['Overview', 'Specs', 'Projects', 'Agents', 'Workflows', 'API Dev'].includes(btn.textContent || ''));
    ...
    describe('Edge Cases', () => {
        ...
        test('should handle undefined currentSection', () => {
            expect(() => renderNavigation(undefined)).not.toThrow();
        });
