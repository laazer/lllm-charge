...
import { DocsIntelligence, DocQuery, DocSearchResult } from '../intelligence/docs-intelligence'
...
export const docsTools = {
  search_developer_docs: {
    ...
    handler: async (params: any, context: DocsToolContext): Promise<string> => {
      try {
  ...
  install_developer_docs: {
    ...
    handler: async (params: any, context: DocsToolContext): Promise<string> => {
      try {
        const docsToInstall: string[] = params.docs
        const forceReindex: boolean = params.force_reindex || false
  ...
  list_available_docs: {
    ...
    handler: async (params: any, context: DocsToolContext): Promise<string> => {
      try {
        const showInstalledOnly: boolean = params.show_installed_only || false
  ...
  get_documentation_status: {
    ...
    handler: async (params: any, context: DocsToolContext): Promise<string> => {
      try {
        const status = await context.docsIntelligence.getDocumentationStatus()
        const queueStatus = await (context.docsIntelligence as any).smartCache?.getQueueStatus() || { queued: [], inProgress: [] }
  ...
  quick_doc_lookup: {
    ...
    handler: async (params: any, context: DocsToolContext): Promise<string> => {
      try {
        const apiOrConcept: string = params.api_or_concept
        const languageOrTool: string = params.language_or_tool
        const includeExamples: boolean = params.include_examples !== false
