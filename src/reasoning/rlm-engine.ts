...
export class RLMEngine {
  private sessions: Map<string, ReasoningSession>
  private pythonPath: string
  private rlmScript: string

  constructor(private config: ReasoningConfig) {
  ...
  async startReasoningSession(query: string, context?: string): Promise<string> {
  ...
  async executeRLMQuery(sessionId: string, query: string, context?: string): Promise<string> {
  ...
  async listSessions(): Promise<ReasoningSession[]> {
  ...
  private async setupRLMBridge(): Promise<void> {
    const bridgeScript = `
#!/usr/bin/env python3
import sys
import json
...
`
  ...
  private parseRLMOutput(sessionId: string, output: string): void {
    const session = this.sessions.get(sessionId)
    if (!session) return
    ...
    try {
      ...
      if (result.metadata && result.metadata.iterations) {
        session.iterations = result.metadata.iterations.map((iter: any, index: number) => ({
          step: index + 1,
          prompt: iter.prompt || '',
