...
export class WorkflowEditor extends EventEmitter {
  ...
  connectNodes(sourceId: string, targetId: string, sourceIndex = 0, targetIndex = 0): void {
    if (!this.state.workflow) return
  ...
  copyNodes(): void {
    if (!this.state.workflow || this.state.selectedNodes.length === 0) return

    const selectedNodes = this.state.workflow.nodes.filter(node => 
      this.state.selectedNodes.includes(node.id)
    )
  ...
  pasteNodes(offset: NodePosition = { x: 50, y: 50 }): void {
    if (!this.state.workflow || this.state.clipboard.nodes.length === 0) return

    const pastedNodes = this.state.clipboard.nodes.map(node => ({
      ...node,
      id: this.generateNodeId(),
      name: this.generateNodeName(node.type),
      position: [node.position[0] + offset.x, node.position[1] + offset.y] as [number, number]
    }))
  ...
  fitToView(): void {
    if (!this.state.workflow || this.state.workflow.nodes.length === 0) return
  ...
  private initializeConfig(): CanvasConfig {
    return {
  ...
  private initializePalette(): NodePalette {
    return {
      categories: [
        {
          name: 'Triggers',
  ...
  private drawConnectionPoints(node: WorkflowNode, x: number, y: number, width: number, height: number): void {
  ...
  private drawConnection(sourceNode: WorkflowNode, targetNode: WorkflowNode): void {
  ...
  // Utility methods
  private screenToWorld(screenPos: NodePosition): NodePosition {
    return {
  ...
  private worldToScreen(worldPos: NodePosition): NodePosition {
    return {
  ...
  private getNodeAtPosition(position: NodePosition): WorkflowNode | null {
    if (!this.state.workflow) return null
  ...
  private moveSelectedNodes(delta: NodePosition): void {
    if (!this.state.workflow) return
  ...
  private updateWorkflowFromCanvas(): void {
    // This would sync any canvas-specific state back to the workflow
    // For now, node positions are already updated directly
  }

  private calculateWorkflowBounds(): { left: number; top: number; right: number; bottom: number; width: number; height: number } {
